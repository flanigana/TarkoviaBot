...
async function loadCategoryPages() {
	...
	for (const cat of categories) {
		promises.push(
			wiki.pagesInCategory('Category:' + cat).then((pages) => {
...
async function initialize() {
	...
	let pagesByCategory = {};
	promises.push(
		wiki.allPages().then((pages) => {
			pageTitles = pages;
			return true;
		})
	);
	promises.push(
		loadCategoryPages().then((pages) => {
...
client.on('message', async (msg) => {
	...
	switch (command) {
		...
		case 'quest_item':
			response =
				'https://static.wikia.nocookie.net/escapefromtarkov_gamepedia/images/1/19/QuestItemRequirements.png/revision/latest';
			break;
	}
