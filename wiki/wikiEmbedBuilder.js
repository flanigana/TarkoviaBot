...
function addEmbedFieldsFromInfo(embed, info) {
  ...
  for (const key of keys) {
    ...
    if (
      key.match(/type|material|penalties|objectives|rewards/gi) ||
      info[key].length > 30
    ) {
...
function getLocationMap($, info) {
  info.Image =
    findMapImage($, /expansion/gi) ||
    findMapImage($, /2D Map|3D Map/gi) ||
    findMapImage($, /map/gi);
  ...
  for (const e of elements) {
    if (e.name == "h2") {
      ...
      if (
        bullet != "" &&
        !bullet.match(
          /^\d+,?\d+ (roubles|dollars|euros) with Intelligence Center Level \d$/gi
        )
      ) {
        bullets.push(bullet);
      }
...
function getMainTableInfo($) {
  let info = {};
  const mainTableRow = $(
    "#va-infobox0-content > .va-infobox-cont > .va-infobox-group > tbody > tr"
  );
      ...
      for (const contentItem of content) {
        ...
        if (list.length > 0) {
          if (
            ...
            contentItem.children[0].type == "text"
          ) {
...
async function getPageInfo(page) {
  return page.html().then((html) => {
    ...
    if (
      page.raw.title.match(
        /customs|woods|factory|interchange|shoreline|reserve|lighthouse|the lab|labs|lab/gi
      )
    ) {
      ...
      ] = `[Here](https://mapgenie.io/tarkov/maps/${interactiveLinkEnd})`;
  ...
  promises.push(
    getPageInfo(page).then((info) => {
      ...
      if (info.Image && info.Image.match(/^https?/gi)) {
        ...
        innerPromises.push(
          addImageMatchingRegex(
            page,
            embed,
            RegExp("slots|inside|internal|layout", "gi")
          )
        );
      }
